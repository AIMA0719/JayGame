...
- **슬롯→경로 거리 (각 변까지):**
  - 모서리(0,5,6,11,12,17): 가까운 2변 ~81px, 먼 2변 ~98+px
  - 상하 가장자리(1-4, 13-16): 상/하변 ~98px, 좌/우변 177~465px
  - 내부 가장자리(7,10): 가까운 1변 177px, 상하변 218px
  - 중앙(8,9): 상하변 218px, 좌우변 273+px
- **사거리 설계 (위치 전략):**
  - MELEE(100~145): 모서리+상하 가장자리만 (내부 슬롯 불가)
  - RANGED COMMON/RARE(160~180): 모서리+상하 가장자리, 내부 불가
  - RANGED HERO(270~280): 슬롯7,10에서 가까운변+상하+대각, 슬롯8,9는 상하+살짝대각
  - RANGED LEGEND(295~320): 슬롯8,9에서 가까운변+상하변 커버
  - RANGED MYTHIC(370~390): 어디서든 전 변 커버 (슬롯8,9에서 좌우상하+가까운코너)
  - RANGED IMMORTAL(420~440): 어디서든 전체 커버 (먼 코너 포함)
